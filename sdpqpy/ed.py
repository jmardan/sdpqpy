...
class EDFermiHubbardModel():
    ...
    def createHamiltonian(self):
        ...
        # efficiently generate the off diagonal part of the Hamiltonian
        hdict = {}
        ...
        for row, vec in enumerate(self.getHilbertSpace()):
            ...
            for j1 in range(V):
                if self.t != 0:
                    for k1 in get_neighbors(j1, self.getLength(),
                                            width=self.getWidth(),
                        ...
                        if newvec is not None:
                            ...
                            H[col, row] += -self.t * sign
                if self.t2 != 0:
                    for k1 in get_next_neighbors(j1, self.getLength(),
                                                 width=self.getWidth(),
                                                 distance=2,
                                                 periodic=self._periodic):
                        j2 = j1 + V
        ...
        print("Hilbert space and Hamiltonian for system of dimension " +
              str(len(H)) + " generated in", time.time() - time0, "seconds")
        return H
    ...
    def getNumberOfDoubleOccupiedSites(self):
        if self.groundstate is None:
            self.solve()
        Pdiag = [np.dot(vec[:self.getSize()], vec[self.getSize():])
                 for vec in self.getHilbertSpace()]
    ...
    def getAnnihiliationOperators(self):
        ...
        if self.annihiliationOperators is None:
            ...
            self.annihiliationOperators = [
                self.createAnnihiliationOperator(j) for j in range(0, 2 * V)]
    ...
    def createMonomialVector(self, variables, monomials):
        ...
        self.monomialvector = []
        with multiprocessing.Pool() as pool:
            monomials = pool.imap(ft.partial(expressionToMatrix,
                                             variables=list(variables),
                                             matrices=self.getAnnihiliationOperators()),
                                  flatmonomials)
            for i, monom in enumerate(monomials, 1):
                self.monomialvector.append(monom)
                sys.stdout.write("\r\x1b[K processed " + str(i) + " of " +
                                 str(monomialsLength) + " monomials in " +
